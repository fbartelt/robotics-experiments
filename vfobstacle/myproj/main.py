...
class Curve:
    ...
    def step(self, dt, t, obstacles=None):
        ...
        for i, s in enumerate(self.s):
            ...
            const_grad = (prev_grad + next_grad) / 2.0
            self.h[index] = hs + dt * (-self.zeta * (hs - h0s) + self.eta * grad - self.beta * const_grad)
    ...
    def dh_dt(self, s, t, obstacles=None, dt=1e-3):
        ...
        next_deform = self.deformation(
            curr_hs, obstacles, t + dt, r=self.r, alpha=self.alpha
        )
...
class GVF(Scene):
    def construct(self):
        ...
        b0 = b0 + (A0 @ np.array([-12.0, -1.0]).reshape(-1, 1)).ravel()
        vel_vec = 0.0 * np.array([0.0, 0.5])  # Velocity vector for the obstacle
        ...
        def aut_trajectory(q0, k_n=k_n, dt=dt):
            ...
            trajectory, xi_ns, xi_ts, xis, ffs, curve_hist = simulate_trajectory(
                q0, t=time, obstacles=obstacle, curve=curveobj, k_n=k_n, dt=dt
            )
        ...
        ).add_updater(update_arrow_wrapper(xi_ns)).update()
        tangent_arrow = Arrow(
            trajectory[0],
            trajectory[0] + 0.2 * UL,
            color=PURE_GREEN,
        ...
        self.play(
            ...
            DrawBorderThenFill(square, run_time=2, lag_ratio=0.05),
            Create(dots_group, run_time=2, lag_ratio=0.05),
        )
        self.wait(1)
...
def simulate_trajectory(q0, t, obstacles, curve, k_n=2.0, k_t=1.0, dt=0.01, steps=500):
    ...
    for i in range(steps):
        ...
        tangent = kt * norm_tan
        ff = curve.dh_dt(s_star, time, obstacles, dt=dt)
