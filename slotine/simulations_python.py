...
num_freq = 2  # Number of frequencies to use in the trajectory
...
# Kd Matrix
Kd = 4e-1*np.diag(np.hstack([(5e4 / N) * np.ones(3), (5e3 / N) * np.ones(3)]))
...
P_o = 30 * np.linalg.inv(np.diag(np.abs(a_i.ravel()) + 1e-2))
P_r = 30 * np.eye(3)
tol = 1e-5
