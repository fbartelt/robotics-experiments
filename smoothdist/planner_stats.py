...
class OptimalPathProblem:
    ...
    def objective(self, x):
        ...
        for i in range(self.N):
            ...
            for j, obs in enumerate(self.obstacles):
                ...
                #     test=self.test,
                # )
                dist_ij, _ = signedEuclideanDistance(p_i, obs.A, obs.b.reshape(-1, 1))
                dists[j] = dist_ij
            # smooth_min_dist = smoothMinList(dists, self.r)
            smooth_min_dist = np.min(dists)
    ...
    def gradient(self, x):
        ...
        for i in range(self.N):
            ...
            smooth_min_grad = grads[:, min_idx].reshape(-1, 1)
            ...
            grad_full = grad_sat * smooth_min_grad  # n x 1
